...
| `DATABASE_URL` | 手順1のプール接続文字列（Marketplace 連携なら自動設定される） |
...
`DJANGO_DEBUG` は未設定（または `0`）のままにする。有効にすると DEBUG 動作になる。`SECRET_KEY` が未設定だと起動時に `ImproperlyConfigured` で失敗する（意図した挙動）。
